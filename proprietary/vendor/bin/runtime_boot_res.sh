...
if [ ! -d ${CUPSS_DEFAULT_PATH}/${RES_PATH} ]; then
...
if [ -d /OP ]; then
    ANI_ROOT_PATH=OP
    DOWNCA_APP_MANAGER_INSTALLATION_FILE=/OP/_COMMON/app-enabled-conf.json
else
...
if [ -d "/OP/_COMMON/media" ]; then
    if [ ${SBP_VERSION} -gt "30" ]; then
        OP_PRELOAD_DONE="/data/system/op_preload_done.ini"
        if [ ! -f ${OP_PRELOAD_DONE} ]; then
            MUSIC_ORIGINAL_FILE_DIR="/OP/_COMMON/media/Preload/LG"
            VIDEO_ORIGINAL_FILE_DIR="/OP/_COMMON/media/Preload"
            PRELOAD_LINK_LOCATION_DIR="/data/media/0/Preload"
            mkdir -p ${PRELOAD_LINK_LOCATION_DIR}

            MUSICLIST=$(ls ${MUSIC_ORIGINAL_FILE_DIR})
            for MUSIC in ${MUSICLIST}; do
                if [ -f ${MUSIC_ORIGINAL_FILE_DIR}/${MUSIC} ]; then
                    if [ "${OP_PRELOAD_TYPE}" == "copy" ]; then
                        cp -rf ${MUSIC_ORIGINAL_FILE_DIR}/${MUSIC} ${PRELOAD_LINK_LOCATION_DIR}/${MUSIC}
                    else
                        ln -sfn ${MUSIC_ORIGINAL_FILE_DIR}/${MUSIC} ${PRELOAD_LINK_LOCATION_DIR}/${MUSIC}
                    fi
                fi
            done

            VIDEOLIST=$(ls ${VIDEO_ORIGINAL_FILE_DIR})
            for VIDEO in ${VIDEOLIST}; do
                if [ -f ${VIDEO_ORIGINAL_FILE_DIR}/${VIDEO} ]; then
                    if [ "${OP_PRELOAD_TYPE}" == "copy" ]; then
                        cp -rf ${VIDEO_ORIGINAL_FILE_DIR}/${VIDEO} ${PRELOAD_LINK_LOCATION_DIR}/${VIDEO}
                    else
                        ln -sfn ${VIDEO_ORIGINAL_FILE_DIR}/${VIDEO} ${PRELOAD_LINK_LOCATION_DIR}/${VIDEO}
                    fi
                fi
            done

            /system/bin/chown -R media_rw:media_rw ${PRELOAD_LINK_LOCATION_DIR}
            /system/bin/chmod -R 0775 ${PRELOAD_LINK_LOCATION_DIR}
...
if [ "${IS_LIVE_DEMO_UNIT}" == "1" ]; then
    ...
    RETAIL_CONTENTS_OP_DIR="/OP/_COMMON/media/Preload/RetailContents"
    ...
    #2. Create RetailConents Directory
    ...
    #3. Permisison Setting
    if [ -d ${RETAIL_CONTENTS_LN_DIR} ]; then
        /system/bin/chown -R media_rw:media_rw ${RETAIL_CONTENTS_LN_DIR}
        /system/bin/chmod -R 0775 ${RETAIL_CONTENTS_LN_DIR}
    fi
fi
...
PRELOAD_LINK_LOCATION_CONTENTS="/data/media/0/Preload/displaycheckcontents"
mkdir -p ${PRELOAD_LINK_LOCATION_CONTENTS}
/system/bin/chown -R media_rw:media_rw ${PRELOAD_LINK_LOCATION_CONTENTS}
/system/bin/chmod -R 0775 ${PRELOAD_LINK_LOCATION_CONTENTS}
